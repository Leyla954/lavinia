...
const CartItem = ({ item }) => {
  ...
  const handleWishlistToggle = () => {
    dispatch(toggleWishlist({ item, isAuthenticated, showAuthModal: () => setModalVisible(true) }));
  };

  return (
    <div className="relative bg-white shadow-lg shadow-green-500/80 rounded-lg p-7 flex flex-col sm:flex-row items-center gap-6 w-full sm:w-[400px]">
      <div className="absolute top-2 right-2 flex gap-3">
        ...
        <CloseOutlined onClick={() => dispatch(removeFromCart(item.id))} className="text-gray-500 text-lg cursor-pointer hover:text-red-500 transition" />
      </div>
      ...
      <div className='flex flex-col flex-grow'>
        ...
        <div className="flex items-center gap-3 mt-3">
          <button onClick={() => dispatch(decreaseQuantity(item.id))} className="shadow-lg shadow-red-600/100 text-black bg-red-50 px-2 py-1 rounded-md text-sm hover:bg-red-100 transition">-</button>
          <span className="text-lg font-bold">{item.quantity}</span>
          <button onClick={() => dispatch(increaseQuantity(item.id))} className="shadow-lg shadow-green-600/100 bg-green-50 px-2 py-1 rounded-md text-sm hover:bg-green-100 transition">+</button>
        </div>
